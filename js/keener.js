 ...
 * 
 * Referrences
 * http://s2terminal.hatenablog.com/entry/2015/10/12/232557
...
let data = [
    ...
    ["45-0", "7-27", "30-3", "52-14", "-", "3-1", "90"]
];

let names, matches, example, strength, player = [], x = [], A, rates, rank;

function H(x) {
    return 0.5 + (Math.sign(x - 0.5) * Math.sqrt(Math.abs(2 * x - 1))) * 0.5;
}

function toDistort(array) {
    let ret = [];
...
$(document).on(`click`, "#reCal", function (e) {
    ...
    if ($("[name=sigm]:checked").val()) {
        console.log("Skew-adjust");
        strength.updateSettings({
            data: A = toDistort(calc_WP()),
            rowHeaders: Names,
            colHeaders: Names
        });
    } else {
